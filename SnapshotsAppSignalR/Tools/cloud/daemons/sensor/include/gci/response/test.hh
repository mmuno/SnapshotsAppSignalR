...
#ifndef TEST_HH_2950912084__H_
#define TEST_HH_2950912084__H_
...
struct channels {
    std::vector<high > high;
    std::vector<medium > medium;
    std::vector<low > low;
    channels() :
        high(std::vector<high >()),
        medium(std::vector<medium >()),
        low(std::vector<low >())
        { }
};
...
namespace avro {
template<> struct codec_traits<c::high> {
    static void encode(Encoder& e, const c::high& v) {
    ...
    static void decode(Decoder& d, c::high& v) {
...
template<> struct codec_traits<c::medium> {
    static void encode(Encoder& e, const c::medium& v) {
    ...
    static void decode(Decoder& d, c::medium& v) {
        if (avro::ResolvingDecoder *rd =
            dynamic_cast<avro::ResolvingDecoder *>(&d)) {
            const std::vector<size_t> fo = rd->fieldOrder();
            for (std::vector<size_t>::const_iterator it = fo.begin();
...
template<> struct codec_traits<c::low> {
    static void encode(Encoder& e, const c::low& v) {
    ...
    static void decode(Decoder& d, c::low& v) {
        if (avro::ResolvingDecoder *rd =
            dynamic_cast<avro::ResolvingDecoder *>(&d)) {
            const std::vector<size_t> fo = rd->fieldOrder();
            for (std::vector<size_t>::const_iterator it = fo.begin();
